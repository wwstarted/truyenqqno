...
$comic_ids = $wpdb->get_col("
    ...
    ORDER BY COALESCE(s.weekly_views, 0) DESC, 
...
<div id="main_homepage" data-ajax-enabled="true" data-filter-type="top-tuan">
    <!-- Section Header -->
    <div class="homepage_tags">
        <h1>
            <p class="text_list_update">
                <i class="fa fa-fire" aria-hidden="true"></i> Top Truyện Tuần
            </p>
        </h1>
        <div class="clear"></div>
    </div>
    ...
    <div class="list_grid_out">
        <ul class="list_grid grid">
            ...
            if ($comics_query->have_posts()):
                ...
                while ($comics_query->have_posts()):
                    ...
                    $view_stats = $wpdb->get_row(
                        $wpdb->prepare(
                            "SELECT weekly_views FROM {$stats_table} WHERE post_id = %d",
                            $post_id
                        )
                    );
                    $view_count = $view_stats ? $view_stats->weekly_views : 0;
            ...
            <?php
                ...
                echo '<li class="no-results"><p>Chưa có dữ liệu lượt xem tuần này.</p></li>';
            endif;
            ?>
        </ul>
    </div>
