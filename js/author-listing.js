/**
 * Author Listing — AJAX Pagination + Sort
 * taxonomy-nettruyen_author.php
 *
 * @package TruyenQQ
 * @version 1.0.0
 */
...
(function () {
  ...
  function init() {
    ...
    sortSelect          = document.querySelector("#author-sort");

    if (!comicsGrid) return;

    /* read author slug from URL path */
    state.authorSlug = getAuthorSlugFromURL();
    ...
    console.log("Author Listing initialized", state);
  }

  /* ── helpers ──────────────────────────────────── */
  function getAuthorSlugFromURL() {
    /* works for both /tac-gia/slug/ and /nettruyen_author/slug/ */
  ...
  async function loadComics() {
    ...
    try {
      const restBase =
        typeof nettruyenAuthorData !== "undefined"
          ? nettruyenAuthorData.restUrl
          : window.location.origin + "/wp-json/nettruyen/v1/comics/author";

      const apiUrl = new URL(restBase);
      apiUrl.searchParams.set("author", state.authorSlug);
      ...
      if (data.success) {
        ...
        /* re-init bookmarks on newly rendered cards */
      ...
      console.error("Author AJAX error:", err);
  ...
  function renderComics(comics) {
    if (!comics || comics.length === 0) {
      comicsGrid.innerHTML = `
        <li class="no-results"><p>Không tìm thấy truyện nào.</p></li>`;
      return;
    }

    comicsGrid.innerHTML = comics.map(function (c) {
  ...
  console.log("Author Listing JS loaded");
})();