...
function toyota_enqueue_assets()
{
    ...
    if (is_404()) {
        ...
        wp_enqueue_script(
            'truyenqq-404-script',
            get_template_directory_uri() . '/js/404.js',
            array(),
            '1.0.0',
            true
        );
    ...
    if (is_page_template('page-theo-doi.php')) {
        ...
        wp_localize_script('theo-doi-js', 'truyenqqConfig', array(
            'apiBase' => esc_url_raw(rest_url('nettruyen/v1')),
            'nonce' => wp_create_nonce('wp_rest'),
            'homeUrl' => esc_url(home_url()),
            'isLoggedIn' => is_user_logged_in(),
            'authEnabled' => defined('TRUYENQQ_AUTH_ENABLED') ? TRUYENQQ_AUTH_ENABLED : true, // ← thêm dòng này
        ));
    }
    ...
    if (is_page_template('page-lich-su.php')) {
        ...
        wp_localize_script('toyota-front-page', 'truyenqqConfig', array(
            ...
            'homeUrl' => esc_url(home_url()),
            'isLoggedIn' => is_user_logged_in(),
            'authEnabled' => defined('TRUYENQQ_AUTH_ENABLED') ? TRUYENQQ_AUTH_ENABLED : true,
        ));
    ...
    if (is_page_template('page-dieu-khoan.php')) {
        wp_enqueue_style(
            'dieu-khoan-style',
    ...
    if (is_page_template('page-chinh-sach.php')) {
        wp_enqueue_style(
            'chinh-sach-style',
            get_template_directory_uri() . '/css/info-pages.css',
            array('toyota-global'),
    ...
    if (is_page_template('page-gioi-thieu.php')) {
        wp_enqueue_style(
            'gioi-thieu-style',
            get_template_directory_uri() . '/css/gioi-thieu.css',
            array('toyota-global'),
            '1.0.0'
        );
...
function toyota_theme_setup()
{
    ...
    register_nav_menus(array(
        ...
        'footer-genres' => __('Footer - Thể Loại', 'toyota-theme'),
        'footer-ranking' => __('Footer - Xếp Hạng', 'toyota-theme'),
        'footer-support' => __('Footer - Hỗ Trợ', 'toyota-theme'),
    ));
}
 ...
 */
add_action('init', 'nettruyen_register_country_taxonomy');
function nettruyen_register_country_taxonomy()
{
    $labels = array(
        'name' => 'Quốc gia',
 ...
 * Detect country from multiple sources with priority
 */
function nettruyen_detect_country($post_id)
{
    ...
    if (
        strpos($title_lower, 'manhwa') !== false ||
        strpos($title_lower, 'korea') !== false ||
        strpos($title_lower, 'hàn quốc') !== false
    ) {
        return 'Korea';
    }

    if (
        strpos($title_lower, 'manga') !== false ||
        strpos($title_lower, 'japan') !== false ||
        strpos($title_lower, 'nhật bản') !== false
    ) {
        return 'Japan';
    }

    if (
        strpos($title_lower, 'manhua') !== false ||
        strpos($title_lower, 'china') !== false ||
        strpos($title_lower, 'trung quốc') !== false
    ) {
        return 'China';
    }
...
/**
 * Auto migrate country for all comics
...
function nettruyen_auto_migrate_country()
{
    ...
    foreach ($comics as $post_id) {
        ...
        if ($detected_country) {
            $term = get_term_by('slug', $detected_country, 'nettruyen_country');
            if ($term) {
...
// rewrite rules for comics

add_filter('register_post_type_args', function ($args, $post_type) {
    if ($post_type === 'nettruyen_comic') {
        $args['rewrite'] = array(
            'slug' => 'truyen-tranh',
            'with_front' => false,
        );
...
// rewrite rules for chapters

function nettruyen_remove_trailing_slash_html($redirect_url, $requested_url)
{
...
add_filter('redirect_canonical', 'nettruyen_remove_trailing_slash_html', 10, 2);